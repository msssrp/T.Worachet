...
router.put('/restaurants/:id' , async(req,res)=>{
    ...
    if(!updateRestaurant || Object.keys(updateRestaurant).length === 0){
        return res.status(400).json({error:"please input id and update infomations"})
    }
    try {
        const updateRestaurantResult = await Restaurant.updateRestaurant(id,updateRestaurant)
